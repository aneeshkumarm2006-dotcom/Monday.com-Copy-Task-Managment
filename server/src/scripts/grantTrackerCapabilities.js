/**
 * grantTrackerCapabilities.js
 *
 * Ticks the two tracker capabilities onto the roles of organisations that
 ...
 * permission lists, so `tracker.view` is absent everywhere and the Delivery tab
 * simply never appears. "Why is the Trackers button missing" would otherwise be
 * the first thing anyone asks.
 *
 * Matches the defaults a NEW workspace is seeded with (see SYSTEM_ROLES in
 * utils/capabilities.js):
 *     admin   → tracker.view, tracker.manage
 *     member  → tracker.view, tracker.manage
 *     viewer  → tracker.view
 *     guest   → nothing (external; a board-wide grid across every client is not
 *               theirs to see)
 ...
 *     node src/scripts/grantTrackerCapabilities.js [--org <orgId>] [--dry-run]
 */
...
const run = async () => {
  ...
  console.log(
    `\n${dryRun ? '[DRY RUN] ' : ''}Granting tracker capabilities across `
...
run().catch((err) => {
  console.error('grantTrackerCapabilities failed:', err);
  process.exit(1);
});
